...
export const Login: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    setError(null);
    try {
      const response = await api.post('/auth/google-login', "mock-google-token-12345");
      dispatch(setCredentials(response.data));
      navigate('/');
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-[#09090b] px-4">
      <div className="w-full max-w-md bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl overflow-hidden p-8">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-zinc-950 dark:text-zinc-50 tracking-tight">Welcome to QuantFloww</h2>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1.5">Enter your credentials to access the terminal</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg py-2.5 font-medium transition-colors shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer mt-2"
          >
            {loading ? (
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-white dark:bg-[#0c0c0f] px-2 text-zinc-500 dark:text-zinc-500">
              Or continue with
            </span>
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 bg-zinc-100 dark:bg-[#18181b] hover:bg-zinc-200 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-200 rounded-lg py-2.5 font-medium transition-colors shadow-sm cursor-pointer disabled:opacity-50"
        >
